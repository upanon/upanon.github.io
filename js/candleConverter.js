...
function setToMonthly( timeData ) {
...
function setToWeekly( timeData ) {

	var time = new Date( timeData );

	time.setHours( 0, 0, 0, 0 );
	
	var day = time.getDay() || 7; // Get current day number, converting Sun. to 7

	if ( day !== 1 ) {

		time.setHours( -24 * ( day - 1 ) );
...
function setToFourHour( timeData ) {

	var time = new Date( timeData );

	var hours = time.getHours();

	var toSubstract = hours % 4;

	var floored = hours - toSubstract;

	time.setHours( floored, 0, 0, 0 );
...
function setToOneMinute( timeData ) {
...
function convertToCandle( data ) {
	...
	for ( var i = 0; i < data.length; i++ ) {
		...
		if ( timeframe === "M" ) {
			...
			time = setToMonthly( data[i].x );

		} else if ( timeframe === "W" ) {

			time = setToWeekly( data[i].x );
